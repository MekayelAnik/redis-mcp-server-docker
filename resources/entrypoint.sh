...
readonly HAPROXY_SERVER_NAME="redis-mcp"
...
start_mcp_server() {
    ...
    REDIS_MAX_MEM_MB="${REDIS_MAX_MEM_MB:-0}"

    local redis_argv=(redis-mcp-server)
    if [ -n "$redis_url" ]; then
        ...
        redis_argv=(redis-mcp-server --url "${redis_url}")
    fi

    # mcp-proxy receives the stdio command as positional args after `--`,
    # so prlimit can prefix the argv list directly.
    if [[ "${REDIS_MAX_MEM_MB}" =~ ^[1-9][0-9]*$ ]] && command -v prlimit >/dev/null 2>&1; then
        local mem_bytes=$((REDIS_MAX_MEM_MB * 1024 * 1024))
        redis_argv=(prlimit "--as=${mem_bytes}" -- "${redis_argv[@]}")
        echo "Redis MCP child memory cap: ${REDIS_MAX_MEM_MB} MiB (prlimit --as)"
    fi
    ...
    echo "Launching Redis MCP with protocol: ${PROTOCOL_DISPLAY}"

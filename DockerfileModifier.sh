...
REDIS_MCP_VERSION=$(cat ./build_data/version 2>/dev/null || exit 1)
REDIS_MCP_PKG="redis-mcp-server==${REDIS_MCP_VERSION}"
...
if [ -e ./build_data/publication ]; then
    # For publication builds, create a minimal Dockerfile that just tags the existing image
    {
        ...
        echo "ARG REDIS_MCP_VERSION=$REDIS_MCP_VERSION"
...
LABEL org.opencontainers.image.description="Redis MCP Server — Model Context Protocol server for managing Redis databases, with mcp-proxy (stdio<->StreamableHTTP/SSE bridge)"
LABEL org.opencontainers.image.source="https://github.com/mekayelanik/redis-mcp-server-docker"
...
# Install redis-mcp-server + mcp-proxy from PyPI in one layer (shared pip cache).
# mcp-proxy replaces supergateway as the stdio<->HTTP bridge (pure Python, no Node).
RUN --mount=type=cache,target=/root/.cache/pip \\
    echo "Installing packages: ${REDIS_MCP_PKG} + ${MCP_PROXY_PKG}" && \\
    pip install --no-cache-dir --break-system-packages ${REDIS_MCP_PKG} ${MCP_PROXY_PKG} && \\
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=60s --retries=3 \\
    CMD sh -c 'wget -q --spider --no-check-certificate \$([ "\$ENABLE_HTTPS" = "true" ] && echo https || echo http)://127.0.0.1:\${PORT:-8030}/healthz'
